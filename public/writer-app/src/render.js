  ...
  view.innerHTML = `
    <div class="lobby">
      ...
      ${state.contextoAnaliseVinculada ? (() => {
        ...
        const IC_PECA = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M16 13H8"/><path d="M16 17H8"/><path d="M10 9H8"/></svg>`;
        const IC_BULB = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg>`;
        const IC_WARN = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0; margin-top:1px"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>`;
        return `
        <!-- Banner: confeccao de peca a partir de analise vinculada — estilo glass do dashboard -->
        <div style="position:relative; overflow:hidden; margin:16px 0 24px; padding:18px 20px; border:1px solid var(--violet-border-soft); border-radius:16px; background:var(--violet-gradient); backdrop-filter:blur(12px); -webkit-backdrop-filter:blur(12px); box-shadow:0 8px 32px rgba(0,0,0,0.40), inset 0 1px 0 rgba(255,255,255,0.06); display:flex; align-items:flex-start; gap:16px; animation:fadeSlide 0.4s ease both;">
          <div aria-hidden="true" style="position:absolute; top:0; left:0; right:0; height:1px; background:linear-gradient(90deg, transparent, var(--violet-glow), transparent);"></div>
          <div style="flex-shrink:0; width:46px; height:46px; border-radius:14px; background:var(--violet-bg); border:1px solid var(--violet-border); color:var(--violet-soft); display:flex; align-items:center; justify-content:center;">${IC_PECA}</div>
          <div style="flex:1; min-width:0;">
            <div style="font-weight:700; color:var(--violet-soft); font-size:10.5px; letter-spacing:2px; text-transform:uppercase;">Confeccionando peça vinculada</div>
            <div style="margin-top:5px; font-size:13.5px; line-height:1.5; color:var(--text-dim);">Cliente: <strong style="color:var(--text); font-weight:600;">${escapeHtml(p1.nome_completo || 'sem nome')}</strong> <span style="color:var(--text-ghost);">·</span> Desconto: <strong style="color:var(--text); font-weight:600;">${escapeHtml(state.contextoAnaliseVinculada.desconto || '—')}</strong></div>
            ${sug ? `<div style="margin-top:10px; display:inline-flex; align-items:center; gap:7px; padding:5px 12px 5px 9px; border-radius:999px; background:var(--violet-bg); border:1px solid var(--violet-border-soft); font-size:12px; color:var(--violet-soft); line-height:1.3;">${IC_BULB}Sugestão: <strong style="color:var(--text); font-weight:600;">${escapeHtml(sug.nome)}</strong>${state.produtoSugeridoMotivo ? `<span style="color:var(--text-mute); font-weight:400;"> · ${escapeHtml(state.produtoSugeridoMotivo)}</span>` : ''}</div>` : ''}
            ${faltando.length > 0 ? `<div style="margin-top:10px; display:flex; align-items:flex-start; gap:8px; padding:9px 12px; background:var(--amber-bg); border:1px solid var(--amber-border); border-radius:10px; color:var(--amber-soft); font-size:11.5px; line-height:1.45;"><span style="color:var(--amber); display:inline-flex;">${IC_WARN}</span><span>Cliente sem <strong>${faltando.map(k=>labels[k]).join(', ')}</strong> — preencha na Etapa 01 e os dados ficam salvos no perfil pra próximas peças.</span></div>` : ''}
          </div>
        </div>
        `;
      })() : ''}
