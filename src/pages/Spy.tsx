...
import {
  ...
  ChevronDown, ShieldAlert, ExternalLink, RefreshCw, ScanLine, User,
} from "lucide-react";
...
interface Analise { id: string; cliente_id: string; status: string; arquivos: any[]; relatorio: string | null; resumo: any; erro: string | null; created_at: string; }
  ...
  const { data: clientes = [] } = useQuery({
    ...
    queryFn: async (): Promise<Cliente[]> => {
      const { data, error } = await supabase
        .from("clientes")
        .select("id, nome, cpf_cnpj, drive_folder_id, drive_folder_url")
        .order("nome");
    ...
    <div className="max-w-6xl mx-auto space-y-5">
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Central de inteligência sobre a base. Escolha um cliente, selecione os extratos no Drive e rode a análise.
          As inferências são probabilísticas, com grau de confiança e evidência.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px,1fr] gap-5">
            ...
            {lista.map((c) => (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 border-b border-white/[0.04] transition-colors ${
                  sel?.id === c.id ? "bg-primary/10" : "hover:bg-white/[0.03]"
                }`}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="text-sm text-foreground truncate">{c.nome}</span>
                  {c.drive_folder_id
                    ? <FolderOpen className="h-3.5 w-3.5 text-emerald-400/70 shrink-0" />
                    : <FolderOpen className="h-3.5 w-3.5 text-muted-foreground/30 shrink-0" />}
                </div>
        ...
        <div className="min-w-0">
          {!sel ? (
            ...
            <SpyWorkspace cliente={sel} userId={user?.id || null} onAny={() => qc.invalidateQueries()} />
          )}
        </div>
      </div>
    </div>
...
function SpyWorkspace({ cliente, userId, onAny }: { cliente: Cliente; userId: string | null; onAny: () => void }) {
  const qc = useQueryClient();
  const [selFiles, setSelFiles] = useState<Set<string>>(new Set());
  const [analisando, setAnalisando] = useState(false);
  useEffect(() => { setSelFiles(new Set()); }, [cliente.id]);

  // Documentos do Drive do cliente
  ...
  const { data: analises = [] } = useQuery({
    ...
    refetchInterval: analisando ? 3000 : false,
  });
  const { data: flags = [] } = useQuery({
    queryKey: ["spy-flags", cliente.id],
    queryFn: async (): Promise<Flag[]> => {
      const { data, error } = await (supabase.from("spy_flag" as any) as any)
  ...
  const analisar = async () => {
    ...
    setAnalisando(true);
    toast.info("Analisando… isso pode levar até um minuto.");
    const { data, error } = await supabase.functions.invoke("spy-analisar", {
      body: { cliente_id: cliente.id, arquivos, created_by: userId },
    });
    await qc.invalidateQueries({ queryKey: ["spy-analises", cliente.id] });
    await qc.invalidateQueries({ queryKey: ["spy-flags", cliente.id] });
    setAnalisando(false);
    setSelFiles(new Set());
    if (error || data?.error) toast.error("A análise falhou — veja o detalhe abaixo.");
    else toast.success(`Análise concluída (${data?.flags ?? 0} flags).`);
    onAny();
  };
      ...
      <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[11px] uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
            <FileText className="h-3.5 w-3.5" /> Documentos no Drive (PDF)
          </p>
          {cliente.drive_folder_id && (
            <button onClick={() => refetchDrive()} className="text-[11px] text-muted-foreground hover:text-foreground inline-flex items-center gap-1">
        ...
        {!cliente.drive_folder_id ? (
          ...
          <>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left flex items-center gap-2.5 px-3 py-2 rounded-lg border transition-colors ${
                      on ? "border-primary/40 bg-primary/10" : "border-white/[0.06] bg-white/[0.02] hover:border-white/[0.14]"
                    }`}
                  >
            ...
            <div className="flex justify-end mt-3">
              <Button onClick={analisar} disabled={analisando || selFiles.size === 0} className="gap-1.5">
                {analisando ? <><Loader2 className="h-4 w-4 animate-spin" /> Analisando…</> : <><ScanLine className="h-4 w-4" /> Analisar {selFiles.size > 0 ? `(${selFiles.size})` : ""}</>}
              </Button>
            </div>
          </>
        )}
    ...
    <div className={`rounded-2xl border bg-white/[0.02] overflow-hidden ${erro ? "border-rose-500/20" : "border-white/[0.07]"}`}>
      <button onClick={() => setAberto((o) => !o)} className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left">
        <div className="flex items-center gap-2 min-w-0">
          {proc ? <Loader2 className="h-4 w-4 animate-spin text-primary" />
            : erro ? <AlertTriangle className="h-4 w-4 text-rose-400" />
            : <ShieldAlert className="h-4 w-4 text-primary" />}
          <span className="text-sm font-medium truncate">
            {proc ? "Analisando…" : erro ? "Falhou" : `Análise · ${flags.length} flags`}
          </span>
          {!proc && !erro && risco && (
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full ring-1 ${RISCO[risco] || "text-muted-foreground bg-white/[0.04] ring-white/10"}`}>
              risco {risco}
            </span>
          )}
        </div>
                    ...
                    {fs.map((f) => (
                      <div key={f.id} className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-2.5 ml-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {typeof f.confianca === "number" && (
                            <span className="text-[10px] text-muted-foreground tabular-nums">{Math.round(f.confianca * 100)}% conf.</span>
                          )}
                        </div>
