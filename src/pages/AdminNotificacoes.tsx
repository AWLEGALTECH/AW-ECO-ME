    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Aqui você <strong>liga ou desliga</strong> cada tipo de evento (vale pra todo mundo).
          Para escolher <strong>quais notificações cada usuário recebe</strong>, use
          {" "}<a href="/admin/usuarios" className="text-primary hover:underline">Administração → Usuários</a>.
          Você (admin) sempre recebe todas.
        </p>
      </header>
      ...
      <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.35)] overflow-hidden">
        ...
        <div className="hidden sm:grid grid-cols-[1fr,auto] gap-6 px-5 py-3 border-b border-white/[0.07] text-[10px] uppercase tracking-[0.15em] text-muted-foreground">
          <span>Evento</span>
        ...
        {isLoading ? (
          ...
          <ul className="divide-y divide-white/[0.05]">
            {rows.map((r) => (
              <li key={r.tipo} className="grid grid-cols-[1fr,auto] gap-x-6 gap-y-1 items-center px-5 py-4">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-foreground">{r.label}</p>
                  <p className="text-[11px] text-muted-foreground">
                    {r.ativo ? "Gerando — quem recebe é definido por usuário" : "Desligada — nenhuma notificação é criada"}
                  </p>
                </div>
